...
type ShapePathFormula = {
	editable?: boolean;
	defaultValue?: number;
	range?: [number, number];
	relative?: string;
	getBaseSize?: (width: number, height: number) => number;
	formula: (...args:number[]) => string;
};
type ShapePathFormulas = {
	[shapeName: string]: ShapePathFormula;
};

export const SHAPE_PATH_FORMULAS: ShapePathFormulas = {
...
export const SHAPE_ARRAY:ShapePoolItem[] = SHAPE_LIST.reduce((acc, cur) => {
	return acc.concat((cur.children as never))
}, [])