...
export const useSandbox = (props: React.PropsWithChildren<ISandboxProps>) => {
  ...
  useEffect(() => {
    if (ref.current && workspace) {
      ...
      ref.current.contentWindow['Slides'] = globalThisPolyfill['Slides']
      ...
      <!DOCTYPE html>
        ...
        <style>
          ...
          ::-webkit-scrollbar-thumb {
            background-color:${getCSSVar('--ld-scrollbar-color')};
            border-radius: 0;
            transition: all .25s ease-in-out;
          }
          ::-webkit-scrollbar-thumb:hover {
            background-color: ${getCSSVar('--ld-scrollbar-hover-color')};
          }
          body{
            margin:0;
            padding:0;
