...
import { isStr, isFn, isObj, isPlainObj } from '@editor/shared'
...
export const IconWidget: React.FC<IIconWidgetProps> & {
  ...
  const takeIcon = (infer: React.ReactNode) => {
    if (isStr(infer)) {
      ...
      if (infer.type === 'svg') {
        return React.cloneElement(infer as unknown as any, {
          height,
          width,
          fill: 'currentColor',
  ...
  return renderTooltips(
    ...
    >
      {takeIcon(props.infer as unknown as React.ReactNode)}
    </span>
  )
})
  ...
  useEffect(() => {
    if (ref.current) {
      ...
      if(!ref.current.shadowRoot) {
        root = ref.current.attachShadow({mode: 'open'})
      } else {
        root = ref.current.shadowRoot
      }
