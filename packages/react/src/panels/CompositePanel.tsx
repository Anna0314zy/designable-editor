...
import { isValid } from "@editor/shared";
...
export const CompositePanel: React.FC<ICompositePanelProps> & {
	...
	const renderContent = () => {
		...
		return items.map((item) => {
			return (
				<div key={item.key} style={{ display: (activeKey === item.key && visible) ? "" : "none" }} className={cls(prefix + "-tabs-content")}>
					...
					<div className={prefix + "-tabs-body"}>{item.children}</div>
				</div >
			)
		});
	};
					...
					return (
						<Comp
							className={cls(prefix + "-tabs-pane", {
								active: activeKey === item.key && visible,
							})}
							key={index}
							href={item.href}
							onClick={(e: any) => {
