...
import { TreeNode, CursorStatus, CursorDragType } from '@editor/core'
import { LayoutObserver } from '@editor/shared'
...
export const useValidNodeOffsetRect = (node: TreeNode) => {
  ...
  const compute = useCallback(() => {
    ...
    if (!isEqualRect((rectRef as unknown as any).current, nextRect as unknown as DOMRect) && nextRect) {
