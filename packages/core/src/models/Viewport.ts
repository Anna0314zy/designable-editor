...
import { action, define, observable } from '@slides/reactive'
...
export class Viewport {
  ...
  get scale() {
    ...
    // 这里因为改了原来的层级，所以计算方式不正常了
    // const viewportElement = this.viewportElement
    // 这里处理的不是很友好， viewportElement 原有逻辑可能都乱了
    const viewportElement = this.viewportElement ? this.viewportElement.querySelector('[class$="-canvas-editor"]') as unknown as HTMLElement : null
    if (!viewportElement) return 1
    const clientRect = viewportElement.getBoundingClientRect()
    const offsetWidth = viewportElement.offsetWidth
    if (!clientRect.width || !offsetWidth) return 1
    return clientRect.width / offsetWidth
  }
