import { action, define, observable, toJS } from '@slides/reactive'
import { uid, isFn, each } from '@editor/shared'
...
export class TreeNode {
  ...
  constructor(node?: ITreeNode, parent?: TreeNode) {
    ...
    const id = node.operation?.workspace.id
    this.id = id || node.id || uid()
    if (parent) {
      this.parent = parent
  ...
  triggerMutation<T>(event: any, callback?: () => T, defaults?: T): T {
    if (this.operation) {
      const result = this.operation.dispatch(event, callback) as unknown as any || defaults
  ...
  getRootNode(): TreeNode {
    ...
    return root.parent.getRootNode()
  }
}
